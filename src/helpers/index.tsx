...
export async function getMarketPrice({ networkID, provider }: IBaseAsyncThunk) {
  const hiro_bnb_address = ohm_busd.getAddressForReserve(networkID);
  const pairContract = new ethers.Contract(hiro_bnb_address, PairContract, provider);
...
export async function  getTokenPriceByPair({ networkID, provider }: IBaseAsyncThunk) {
  try {

    const kageContrat = new ethers.Contract(addresses[networkID].HIRO_ADDRESS as string, ierc20Abi, provider);
    const pairContrat = new ethers.Contract(addresses[networkID].WBNB_HIRO_ADDRESS as string, ierc20Abi, provider);
    
    const poolBalance = await kageContrat.balanceOf(addresses[networkID].WBNB_HIRO_ADDRESS);
    const wbnbContract = new ethers.Contract(addresses[networkID].WBNB_TOKEN_ADDRESS, ierc20Abi, provider);
    const bnbInPool = await wbnbContract.balanceOf(addresses[networkID].WBNB_HIRO_ADDRESS);
    const derivedBNB = parseFloat(ethers.utils.formatEther(bnbInPool)) / parseFloat(ethers.utils.formatUnits(poolBalance, 9));
    ...
    return {tokenPrice: 0, derivedBNB: 0};
  }
}
